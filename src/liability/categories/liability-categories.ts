...
const fileUrl = `${folder}/liability-categories.csv`;
...
export const getCategories = (): LiabilityCategory[] => {
    if (!existsSync(fileUrl)) return [];
    const content = readFileSync(fileUrl, 'utf-8');
    const categories = parse(content, { header: true }).data as LiabilityCategory[];
    return categories;
}

const _saveCategories = (categories: LiabilityCategory[]) => {
    const content = unparse(categories, { header: true });
    writeFileSync(fileUrl, content);
};

const _outputCategories = (categories: LiabilityCategory[]) => {
